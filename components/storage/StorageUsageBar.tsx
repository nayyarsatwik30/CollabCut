...
export function StorageUsageBar({ usedBytes, loading, variant = 'full' }: StorageUsageBarProps) {
  const usedGB = (usedBytes ?? 0) / BYTES_PER_GB
  const capGB = DEFAULT_EDITOR_STORAGE_CAP_GB
    ...
    return (
      <div>
        <div className="flex items-center justify-between mb-1">
          <span className="text-[10px] text-th-muted font-mono uppercase tracking-wide">Storage</span>
  ...
  return (
    <div className="p-6 rounded-th-lg border border-th-border bg-th-surface space-y-3">
      <div>
        <span className="text-th-muted block text-[11px] font-mono uppercase mb-1">Storage usage</span>
