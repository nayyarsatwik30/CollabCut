...
function Header({ agency = false }: { agency?: boolean }) {
  return <header className="cc-header"><Logo /><nav className="cc-nav" aria-label="Main navigation"><Link href={agency ? '/' : '/agencies'}>{agency ? 'For editors' : 'For agencies'}</Link><a href="#workflow">How it works</a><a href="#pricing">Pricing</a></nav><Link className="cc-header-cta" href="#start">{agency ? 'Talk to us' : 'Start for ₹349'} <ArrowUpRight size={15} /></Link></header>
}
