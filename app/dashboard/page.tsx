...
export default function DashboardPage() {
  ...
  const createProject = async () => {
    ...
    const res = await fetch('/api/projects', {
      ...
      body:    JSON.stringify({ name: newName.trim(), client: newClient.trim(), emoji: '🎬' }),
    })
    if (res.ok) {
      const data = await res.json()
        ...
        <div className="h-13 shrink-0 bg-th-surface border-b border-th-border flex items-center gap-3 px-6">
          ...
          <div className="flex items-center gap-0.5 p-0.5 rounded-th-sm bg-th-surface-alt border border-th-border">
            {([['grid', Grid3X3], ['list', List]] as const).map(([v, Icon]) => (
              <button key={v} onClick={() => setView(v)} className="p-1.5 rounded btn-press transition-colors"
                style={{ background: view === v ? 'var(--th-surface)' : 'transparent', color: view === v ? 'var(--th-text)' : 'var(--th-muted)' }}>
                <Icon size={14} />
              </button>
            ))}
          </div>
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm">
            <div className="bg-th-surface border border-th-border rounded-th-lg p-6 w-full max-w-sm shadow-panel animate-slide-up">
              ...
              <div className="space-y-3 mb-5">
                <div>
                  <label className="block font-mono text-[10px] uppercase tracking-wider text-th-muted mb-1.5">Project name *</label>
                  <input value={newName} onChange={(e) => setNewName(e.target.value)} placeholder="Wedding Promo — Singh & Mehta"
                ...
                <div>
                  <label className="block font-mono text-[10px] uppercase tracking-wider text-th-muted mb-1.5">Client name</label>
                  <input value={newClient} onChange={(e) => setNewClient(e.target.value)} placeholder="Private Client"
